...
for i=1:length(Tref),
    for j=1:length(w),
        ...
        Pout(i,j)=w(j)*T(i,j);
        Pcopper(i,j)=(Curr(i,j)/idyn)^2.2*0.02;
        ...
        Eta(i,j)=min(1,(max(0.01,(abs(Pout(i,j))/(eps+abs(Pout(i,j))+Ploss(i,j)))))); 
    end
end

Tem = Tem_max.*Tref/max(Tref);
Wem = wem_max.*w/max(w);
Pem = [0:Pem_max/25:Pem_max];
...
for i=2:length(Pem),
    for j=2:length(Tem)
        ...
        if wtemp > wem_max
            ...
            eta(j) = interp2(Wem,Tem,EtaEM,wtemp,Ttemp,'spline');     %linear');
        end
        if eta(j) > PtoTem((i),3)
            PtoTem((i),2) = Tem(j);
            PtoTem((i),3) = eta(j);
